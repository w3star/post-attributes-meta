...
class PAM_Explorer_v1101 {
    ...
    public function sanitize_rating( $val )      { $val = (int) $val; return max(0, min(5, $val)); }
    public function sanitize_beauty( $val )      { $val = (int) $val; return max(0, min(5, $val)); }
    public function sanitize_duration_min( $val ){ $val = (int) $val; return max(0, $val); }
    ...
    // ---------- SVG ICONS ----------
    private function svg_star_filled() { return '<svg class="pam-icon" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M12 17.27L18.18 21 16.54 13.97 22 9.24l-7.19-.62L12 2 9.19 8.62 2 9.24l5.46 4.73L5.82 21z"/></svg>'; }
    private function svg_star_empty() { return '<svg class="pam-icon" viewBox="0 0 24 24" aria-hidden="true"><path fill="none" stroke="currentColor" stroke-width="1.5" d="M12 17.27L18.18 21 16.54 13.97 22 9.24l-7.19-.62L12 2 9.19 8.62 2 9.24l5.46 4.73L5.82 21z"/></svg>'; }
    private function svg_mountain()   { return '<svg class="pam-icon" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M3 19h18L14 7l-2 3-2-3L3 19z"/></svg>'; }
    private function svg_sun()        { return '<svg class="pam-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="4" fill="currentColor"/><g stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><line x1="12" y1="2" x2="12" y2="5"/><line x1="12" y1="19" x2="12" y2="22"/><line x1="2" y1="12" x2="5" y2="12"/><line x1="19" y1="12" x2="22" y2="12"/><line x1="4.5" y1="4.5" x2="6.7" y2="6.7"/><line x1="17.3" y1="17.3" x2="19.5" y2="19.5"/><line x1="17.3" y1="6.7" x2="19.5" y2="4.5"/><line x1="4.5" y1="19.5" x2="6.7" y2="17.3"/></g></svg>'; }
    private function svg_stopwatch()  { return '<svg class="pam-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="14" r="7" fill="none" stroke="currentColor" stroke-width="1.5"/><rect x="10" y="2" width="4" height="2" fill="currentColor"/><line x1="12" y1="14" x2="15" y2="10.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>'; }

    private function icons_group( $icon_html, $count ) { $out = '<span class="pam-icongroup">'; for ( $i=0; $i<$count; $i++ ) $out .= $icon_html; return $out.'</span>'; }
    private function stars_html( $rating ) { $rating = max(0, min(5, (int)$rating)); $out = '<span class="pam-icongroup" aria-label="Rating '.$rating.' von 5">'; for ( $i=1; $i<=5; $i++ ) $out .= ($i <= $rating) ? $this->svg_star_filled() : $this->svg_star_empty(); return $out . '</span>'; }
    private function duration_text( $minutes ) { $minutes = (int) $minutes; if ( $minutes <= 0 ) return '—'; $h = intdiv( $minutes, 60 ); $m = $minutes % 60; if ( $h > 0 && $m > 0 ) return $h . ' h ' . $m . ' min'; if ( $h > 0 ) return $h . ' h'; return $m . ' min'; }
...
<?php
    ...
    public function render_block_explorer( $attributes, $content, $block ) {
        ...
        $diff_vals = []; for($i=$d_from_i;$i<=$d_to_i;$i++) $diff_vals[] = $idx2lab[$i];
        ...
        switch ( $sort ) {
            ...
            case 'rating_desc':  $meta_key = 'pam_rating';      $orderby = [ 'meta_value_num' => 'DESC', 'date' => 'DESC' ]; break;
            case 'rating_asc':   $meta_key = 'pam_rating';      $orderby = [ 'meta_value_num' => 'ASC',  'date' => 'DESC' ]; break;
            case 'beauty_desc':  $meta_key = 'pam_beauty';      $orderby = [ 'meta_value_num' => 'DESC', 'date' => 'DESC' ]; break;
            case 'beauty_asc':   $meta_key = 'pam_beauty';      $orderby = [ 'meta_value_num' => 'ASC',  'date' => 'DESC' ]; break;
            case 'dur_asc':      $meta_key = 'pam_duration_min';$orderby = [ 'meta_value_num' => 'ASC',  'date' => 'DESC' ]; break;
            case 'dur_desc':     $meta_key = 'pam_duration_min';$orderby = [ 'meta_value_num' => 'DESC', 'date' => 'DESC' ]; break;
        ...
        if ( count($diff_vals) < 3 ) { $meta_query[] = [ 'key' => 'pam_difficulty', 'value' => $diff_vals, 'compare' => 'IN' ]; }
        ...
        $idxLabel = function($i){ return $i===1?'Leicht':($i===2?'Mittel':'Schwer'); };
    ...
    <?php if ( $q->have_posts() ) : ?>
      <div class="pam-cards">
        <?php while ( $q->have_posts() ) : $q->the_post();
          ...
          $diff_count = $diff === 'hard' ? 3 : ( $diff === 'medium' ? 2 : ( $diff === 'easy' ? 1 : 0 ) );
        ?>
        <a class="pam-card" href="<?php echo esc_url($perma); ?>">
          <div class="pam-card__title"><?php echo esc_html($title); ?></div>
          <div class="pam-card__meta">
            <span class="pam-badge"><?php echo $this->stars_html($rating); ?></span>
            <span class="pam-badge"><?php echo $diff_count ? $this->icons_group($this->svg_mountain(), $diff_count) : '—'; ?></span>
            <span class="pam-badge"><?php echo $beauty ? $this->icons_group($this->svg_sun(), $beauty) : '—'; ?></span>
            <span class="pam-badge"><span class="pam-icongroup"><?php echo $this->svg_stopwatch(); ?></span> <?php echo $this->duration_text($duration); ?></span>
          </div>
        </a>
